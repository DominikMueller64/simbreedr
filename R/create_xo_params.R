...
#'
#' @examples
#' create_xo_params(L = c(50.5, 100.1, 133.5))
#'
#' @export
create_xo_params <- function(L, m = 10L, p = 0.0,
  ...
  if (obligate_chiasma)
      Lstar <- vapply(X = L, FUN = calc_Lstar, FUN.VALUE = numeric(1L), m = m, p = p)
  else {
    Lstar <- L
  }
  ret$Lstar <- Lstar
...
# #' @export
# create_xo_simulators <- function(L, m=10, p = 0.0,
#                                  obligate_chiasma = FALSE) {
#
#   mk <- function(...) {
#   lapply(list(...), force) # force promise to be instantiated so it doesn't keep another environment alive
#   function() .sim_crossovers(...)
# }
#   ret <- vector('list', length(L))
#   for (i in seq_along(L)) {
#     Lstar <- L[i]
#     if (obligate_chiasma)
#       Lstar <- calc_Lstar(L[i], m, p)
#
#     ret[[i]] <- mk(L[i], m, p, obligate_chiasma, Lstar)
#     # ret[[i]] <- function(L = L[i], Lstar = Lstar) {
